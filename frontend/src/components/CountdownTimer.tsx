...
export function CountdownTimer({ targetIso, label, completedLabel = "Closed", onComplete }: CountdownTimerProps) {
  const remainingMs = useCountdown(targetIso, onComplete);
  ...
  return (
    <div>
      ...
      <p className={`font-mono text-3xl font-bold tracking-tight ${isUrgent ? "text-red-600" : "text-slate-900"}`}>
        {remainingMs > 0 ? formatDuration(remainingMs) : completedLabel}
      </p>
    </div>
  );
}
